...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            % IMPORTANT: cur_scans: the measurement for this particle (distance to closest obstacle)
            ...
            % Step 2: Get the expected measurement for gaussian likelihood reweighting
            % case a) we need to check if the endpoint is within the map. If it isn't, instead of skipping here can take the measurement as the maxrange
            % case b) the endpoint is in the map but is not an obstacle: the expected measurement is also the maxrange.
            ...
            % Check if inside:
            if x_endpt_map < 1 || x_endpt_map > ognx || y_endpt_map < 1 || y_endpt_map > ogny || ogp(y_endpt_map, x_endpt_map) < 0.5
                expected = y_laser_max;
            else
                % case c) In the map and obstacle was detected at scan location:: the expected measurement is the euclidean distance from robot (i.e. particle position) to the obstacle
                dx = x_endpoint - x_particle(n);
                dy = y_endpoint - y_particle(n);
                expected = sqrt(dx^2 + dy^2);
            end
            
            % Reweight by passing through Gaussian Likelihood: x = measurement, mu = expected measurement. w_gain is the 2pi sigma of the gaussian
            w_particle(n) = w_particle(n) * exp(-0.5 * ((cur_scans - expected)^2 / laser_var)) / w_gain;
